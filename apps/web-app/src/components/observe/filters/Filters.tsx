...
export default function Filters({
  ...
  // This is a simplified fix that doesn't require changing the FilterSchema
  // You only need to modify the hasFilterChanged function in your Filters.tsx component

  // Replace just this function in your component:
  const hasFilterChanged = useMemo(() => {
    try {
      // Create safe objects with all required properties
      const safeOriginalFilter = {
        agentId: originalFilter?.agentId || [],
        lookbackPeriod: originalFilter?.lookbackPeriod || {
          label: "24 hours",
          value: 86400000,
        },
        chartPeriod: originalFilter?.chartPeriod || 3600000,
        ...(originalFilter || {}),
      };
      ...
      const safeFilter = {
        ...
        chartPeriod: filter?.chartPeriod || 3600000,
        ...(filter || {}),
      };

      // Create new objects without evalSets and alerts
      const cleanOriginal = { ...safeOriginalFilter };
      const cleanFilter = { ...safeFilter };
      ...
      >
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            <Select
              value={filter.lookbackPeriod?.value?.toString() || lookbackPeriods[0].value.toString()}
              onValueChange={(value) => {
                setFilter({
                  ...filter,
