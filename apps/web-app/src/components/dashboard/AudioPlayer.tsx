...
>(function AudioPlayer(
  ...
  const { data: audioUrlData, isLoading: isLoadingAudioUrl } = api.call.getAudioUrl.useQuery(
    { callId: call.id },
    { enabled: !!call.id, refetchOnWindowFocus: false }
  );
  ...
  // Load the audio visualizer - FIXED: Ensure cleanup function is returned
  useEffect(() => {
    const audioVisualizer = document.getElementById(audioVisualizerId);
    if (audioVisualizer && audioUrlData?.url) {
      const _wavesurfer = WaveSurfer.create({
        container: `#${audioVisualizerId}`,
        url: audioUrlData.url, // Use the signed URL here
        height: small ? 50 : 100,
        splitChannels: [
          { height: small ? 25 : 50 },
          { height: small ? 25 : 50 },
    ...
    // FIXED: Always return undefined or a function
    return undefined;
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [audioVisualizerId, audioUrlData?.url, small]);
  ...
  const downloadAudio = useCallback(async () => {
    if (!audioUrlData?.url) return;

    try {
      const response = await fetch(audioUrlData.url);
